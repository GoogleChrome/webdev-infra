...
export function matchArrayType(type) {
  ...
  if (type.type === 'reflection') {
    const literal = internalMatchTypeLiteral(type);
    if (literal) {
 ...
 */
export function internalMatchTypeLiteral(type) {
  if (type.type !== 'reflection') {
    return;
  }
 ...
 * }=}
 */
export function matchTypeLiteral(reflection) {
  ...
  let {type} = reflection;
  if (!type) {
    if (reflection.kind !== typedocModels.ReflectionKind.TypeLiteral) {
      return;
    }
    type = new typedocModels.ReflectionType(reflection);
  }

  // This could just be a type literal on its own.
  const internal = internalMatchTypeLiteral(type);
  if (internal) {
  ...
  const firstTypeLiteral = internalMatchTypeLiteral(t.types[0]);
  ...
  for (let i = 1; i < t.types.length; ++i) {
    const restTypeLiteral = internalMatchTypeLiteral(t.types[i]);
    if (!restTypeLiteral) {
